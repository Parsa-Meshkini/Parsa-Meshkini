# 👋 Welcome to My GitHub

## About Me
- 🎓 Computer Science student at York University
- 💼 Passionate about backend development & scalable systems
- 🚀 Building projects and solving problems daily
- 🎯 Seeking Summer 2025/2026 Software Engineering internships

## 📊 Quick Stats
- **365+ LeetCode solutions**
- **10+ repositories** with diverse tech stacks
- **Full-stack projects** from frontend to DevOps
- **Problem-solver** with strong system design knowledge

## 🛠️ Tech Stack

**Languages:** Python, JavaScript, TypeScript, Java, SQL, Bash  
**Frontend:** React, HTML5, CSS3, TypeScript, Flutter  
**Backend:** Django, FastAPI, Node.js, REST APIs  
**Databases:** PostgreSQL, MongoDB, MySQL, SQLite  
**DevOps & Cloud:** Docker, AWS, Azure, GitHub Actions  
**Tools & Platforms:** Git, Postman, Linux, VS Code  

## 📌 Featured Projects

Check my pinned repositories below to see my best work! Each project demonstrates:
- Clean, maintainable code
- Professional documentation
- Real-world problem solving
- Full-stack capabilities

## 🎯 What I'm Currently Doing
- ✅ Solving LeetCode problems daily 
- ✅ Building full-stack web applications
- ✅ Learning cloud architecture & DevOps practices
- ✅ Contributing to open-source projects
- ✅ Preparing for internship opportunities

## 🔗 Connect With Me
- 💼 **LinkedIn:** [linkedin.com/in/parsameshkini](https://linkedin.com/in/parsameshkini)
- 🌐 **Portfolio:** [parsameshkini.com](https://parsameshkini.com)
- 📧 **Email:** parsa@parsameshkini.com
- 📱 **Instagram:** [@pameshkini](https://instagram.com/pameshkini)

## 📚 Latest Activity
- Daily contributions to LeetCode solutions
- Maintaining and improving existing projects
- Learning new technologies and frameworks

---

*Passionate about building great software. Always learning. Open to opportunities.* 🚀
